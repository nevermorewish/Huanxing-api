...
export function LogSettingsSection({
  ...
  const form = useForm<LogSettingsFormValues>({
    ...
    defaultValues: {
      ...
      LogDetailEnabled: defaultDetailEnabled,
    },
  })
  ...
  }, [defaultDetailEnabled, defaultEnabled, form])
  ...
  const onSubmit = async (values: LogSettingsFormValues) => {
    ...
    if (values.LogDetailEnabled !== defaultDetailEnabled) {
      updates.push(
        updateOption.mutateAsync({
          key: 'LogDetailEnabled',
          value: values.LogDetailEnabled,
        })
      )
    }
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem className='flex flex-row items-start justify-between rounded-lg border p-4'>
                <div className='space-y-0.5 pe-4'>
                  <FormLabel className='text-base'>
                    {t('Detailed request logging')}
                  </FormLabel>
                  <FormDescription>
                    {t(
                      'Store request payloads and upstream responses for administrator troubleshooting. Disable this to avoid recording request data.'
                    )}
                  </FormDescription>
                </div>
                <FormControl>
